...
If one is interested in retrieving the model coefficients, one can use the `tidy()` function as shown below:

```{r}
tidy(fit) %>% filter(.model=="LSS")
```

As revealed by the output above, all estimates in the LSS model are statistically significant. The plot below shows the fit of the models with the blue line representing the LS model and the red line the DAMPEDS model.
...
Here, once again the LSS model seems to perform the best as it provides the lowest values. Among the ETS models, the ALGO model now stands out.
