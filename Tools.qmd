...
Let's go back to our simple example and assume that the bookstore has more insight on the demand of the books. In particular, assume that if the book is either Frankenstein or Dracula, the probability of a customer buying is 0.9 (the probability of the other books remains at 0.7). We can now modify our demand simulation using a loop and a conditional.
...
Note that the inner conditional checks whether the titles are either Frankenstein or Dracula. If so, then the random binomial number is drawn with the probability set at $0.9$, if not the it is drawn with probability $0.7$. The for loop, goes through all the books in the series one by one and adds a simulated demand.

Here is our data frame with the new simulated values.
