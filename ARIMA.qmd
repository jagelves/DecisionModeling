...
Note the resemblance of the ACF of the simulated data to that of the personal income growth. A decaying ACF with a strong correlation of lag 1. Now let's take a look at the PACF.
...
Once again, if you compare the PACF to the one in personal income growth you can see the resemblance in that there is one significant spike and all other partial autocorrelations are not statistically different from zero. This allows us to confirm that we can model personal income growth with an AR(1) process.

## Modeling and Residuals

Let's model personal income growth using the AR(1) model. We'll also estimate a Least Squares model to compare. Recall, that we can estimate these models by using the `model()` function and retrieve the coefficients with the `coef()` function.
...
<center> $y_t=\mu + \theta \epsilon_{t-1} + \theta \epsilon_{t-2} + \theta \epsilon_{t-3} +...$ </center>

where $\mu$ is the mean of the time series, and $\epsilon$ is the error term. In general, the MA process assumes that the current observation is a linear combination of the weighted sum of past error terms. The error terms capture the unpredictable and random fluctuations in the time series that are not accounted for by the AR model. Below we simulate an MA(1) process and generate the ACF and PACF plots.
...
Interestingly, the graph above looks like white noise. A very ragged and unpredictable pattern that fluctuates around a mean of approximately zero. However, let's take a look at the ACF and PACF.
...
When comparing to the simulated values, the ACF of TCM5Y reflects the same pattern. One spike, and all other lags not significantly different from zero. The PACF doesn't exactly replicate the one from the simulation, but the pattern created is the same. Mainly, there is a decaying PACF, a large lag 1 partial autocorrelation, followed by smaller ones until they all are insignificant. As with the MA(1) process, we can model the series, check that the errors are white noise and then use the model to forecast. 
...
ARIMA combines these three components to forecast future values in a time series by considering its own past, removing trends, and accounting for past errors in predictions.

## Revisiting Chilango's Restaurant

Let's apply the models we have learned so far to the avocado data and see if we can provide better forecasts. The code below loads the data and creates the training and test set.
...
Next, we will estimate the LS, ETS and ARIMA models. We will allow an algorithm to specify the parameters of both the ETS and ARIMA models by using the `ETS()` and `ARIMA()` functions.
...
The coefficients for the ARIMA model can be easily retrieved by using the `tidy()` or `coef()` functions.

```{r}
fit %>% coef()
```

The models suggested by the algorithms are fairly simple. The ETS model suggested is a SES with a smoothing parameter of about $0.65$. As for the ARIMA model, an MA(1) process is suggested with an estimate of $-0.28$. Given this result we would expect one spike on the ACF and decaying PACF. Below are the graphs.
...
These patterns look more like a AR(1). We'll put this model forth as well.
...
Let's look at the accuracy vs. the test set.
```{r}
