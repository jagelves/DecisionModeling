...
In the previous section, we established that growth of personal income in California has a decaying ACF and a single significant spike (at lag 1) in the PACF. These patterns can be generated with an AR(1) model. Specifically, the AR(1) model is of the form:

<center>$y_t=c+\phi y_{t-1}+ \epsilon_t$</center>

where $c$ is a constant, $\phi$ is thw lag coefficient, $y_{t-1}$ is the first lag of $y$, and $\epsilon$ is random error. Note that this model uses the first lag of the series as the single independent variable. The AR model can be extended to include more lags of $y$, and in general, it would be called an $AR(p)$ model, where $p$ is the largest lag included. Since, we include only the first lag of the $y$ variable in the specification above, we refer to the model as an AR(1). Below we simulate some data based on the AR(1) model.
...
Now we can use the data generated to see what the ACF looks like for a AR(1) process. This will allow us to visually identify the AR(1).
...
Note the resemblance of the ACF of the simulated data to that of the personal income growth. There is a decaying ACF with a strong correlation of lag one and subsequent lower correlations as the lag gets larger. Now let's take a look at the PACF.
...
The estimated coefficient ($\phi=0.7$) for the AR(1) process is equal the one used in our simulation. However, the estimated constant is 1.8. If we changed the constant to 1.8, the simulation would resemble the PI growth data better. That is, the AR model selects the constant and lag coefficient ($\phi$) such that it fits the data.

If the AR(1) process correctly describes the series, the errors should behave like white noise. That is because the model should pick up all of the systemic variation, and leave only random error. To inspect the errors we can use the `augment()` function. The ACF of the error is displayed below.
...
Note how the ACF of the errors from the AR(1) model resemble white noise. This suggests that we have identified the systematic component of the series as being an AR(1) process. In other words, there is nothing left to model since the errors are entirely random. This is not the case for the LS model since we still observe some significant spikes (lag 1 and lag 13) in the ACF function.
...
The PACF once again shows no pattern for the residuals of the AR(1) model and some significant lags for the LS model. Once again, the AR(1) model correctly identifies the data generating process in the time series.

### Model Selection {.unnumbered}

We can choose between these two models by looking at the AIC, AICc, or BIC.
...
<center>$y_t=\mu + \theta \epsilon_{t-1}$</center>

where $\mu$ is the mean of the time series, and $\epsilon$ is the error term. As with the AR model, further error term lags could be included in the MA. An MA process with highest lag of $q$ is called an MA(q) process. The MA process assumes that the current observation is a linear combination of the weighted sum of past error terms. The error terms capture the unpredictable and random fluctuations in the time series that are not accounted for by the AR model. Below we simulate an MA(1) process and generate the ACF and PACF plots.
...
The pattern shown in the ACF is one significant spike in the first lag, and a decaying PACF as lags get larger. This is the opposite of the AR(1) process where we had a decaying ACF and one spike in the PACF. We can use the MA(1) process to model series that exhibit this pattern.
...
The patterns shown are textbook AR(1). Hence, the inclusion of the model in the set of candidates. The code below forecasts the series 27 periods ahead and plots.
...
fit %>% forecast(new_data = calits_test) %>% 
  ...
  tab_style_body(
    ...
    targets ="row")
```
The AR(1) and LS models seems to be the best models. Interestingly, one could imagining combining the forecasts of these models to obtain an even better forecast of the data.
