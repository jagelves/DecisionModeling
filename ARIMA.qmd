...
The p-value confirms that Tesla deliveries are non-stationary for a $5$% confidence level. However, it can't be rejected at the $1$% level. I what follows we'll try to model the series by assuming that the time series is stationary.
...
deliveries %>%
  ...
  filter_index("2016 Q1"~"2021 Q4") -> deliveries_train

deliveries %>%
  as_tsibble(index=period, regular=T) %>% 
  filter_index("2022 Q1"~"2022 Q4") -> deliveries_test
```
...
```{r}
deliveries_train %>%
  ACF(lag_max = 12, deliveries) %>% autoplot()+theme_classic()
```

The plot shows that the correlation of the series with its first lag is strongest, and that there is continuous decay in the strength of the correlation as the lags get larger.
...
```{r}
deliveries_train %>%
  PACF(lag_max = 12, deliveries) %>% autoplot()+theme_classic()
```

The graph shows that the series has a strong correlation only with its first lag. Specifically, lag 2 and 3 seemed to have been correlated with the series (see ACF), but this was mainly because of the influence of lag 1.

## The AR(p) model

In the previous section we identified the deliveries of Tesla cars to have a decaying ACF and a single significant spike (at lag 1) in the PACF. This pattern can be generated with an AR(1) model. Specifically, an AR(1) model is of the form:
...
Once again, if you compare the PACF to the one in the deliveries of Tesla you can see the resemblance in that there is one significant spike and all other partial autocorrelations are not statistically different from zero. We can model Tesla deliveries with an AR(1) process.
