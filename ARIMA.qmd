...
In this section we will be introducing important concepts for the analysis of time series using the ARIMA model. The ARIMA model is essential in any forecaster's toolbox as it performs well in forecasting series with time dependencies and seasonality. The concepts of white noise, stationarity, autocorrelation and partial autocorrelation are all essential in understanding the modeling procedure.

## Preliminaries

### White Noise {.unnumbered}

In time series, **white noise** refers to a sequence of random data points that have no correlation to each other. This process is used as a benchmark for other types of time series data that exhibit patterns or trends. We can generate a white noise process by using the normal distribution with a mean of zero and a constant variance. Below we create a tsibble with the simulated data.
...
x="",y="") + geom_hline(yintercept = 0, col="gray", lwd=1.5)
```
A very ragged pattern is shown in the graph above. The series behaves erratically, but it always fluctuates around a mean of 0 and and keeps a standard deviation of one. Such a series is unpredictable, so the best one can do is to describe it by its mean and standard deviation. 
...
Deliveries have mostly been in an upward trend, which makes sense as the company is currently growing. This series seems to not be stationary since it crosses the mean once and never revisits it. An easy way to make the series stationary (make it fluctuate around the mean) is to find differences. Below is the graph of the first difference of the series.
...
The series fluctuates around the mean of zero, but unlike the white noise process behaves less erratic. You will notice that some periods are constantly high, while others low. For example, following the lows at the beginning of the year, deliveries seem to increase sharply. There seems to be correlations between the quarters or time dependencies. We will explore more of this below once we look at autocorrelations and partial autocorrelations.  
...
PI_train %>%
  ...
  autoplot() + theme_bw() + labs(x="", y="ACF")
```
...
```
Interestingly, lag 18 shows a negative correlation with the series. It is important to note that correlations can happen by chance even if we construct the series from a random process.
...
```{r}
tsibble(y=y,period=seq(1,length(y)),index=period) %>% ACF(lag_max = 12, y) %>% autoplot()+theme_bw()+labs(x="",y="ACF")
```
...
```{r}
tsibble(y=y,period=seq(1,length(y)),index=period) %>% PACF(lag_max = 12, y) %>% autoplot()+theme_bw()+labs(x="",y="PACF")
```
...
Let's model personal consumption using the AR(1) model. We'll also estimate a Least Squares model to compare. Recall, that we can estimate these models by using the `model()` function.
