...
The table records the estimates for all the benchmarks discussed in @sec-Bench. The naive method has no entry, as the forecast is created by using the previous period's observed value. Below we illustrate the fit of the least squares model by the red line and the Naive model by the orange line.
...
The graph illustrates how closely the Naive method follows the data. This might seem like a good model, but consider how the heuristic is making a mistake every period since average prices are constantly changing every week. Furthermore, consider how the Naive prediction does not explain the series governing process. The LS model, on the other hand, does provide some insight into a force that is influencing the data—mainly a rising trend. We can use characteristics such as a trend or seasonality to forecast a series.
...
\n
How we decide to aggregate our errors will determine our measure of accuracy. For example, if we follow the same procedure as the one for MSE's but find the square root, we have calculated the RMSE. Below you will find a collection of accuracy measures for our benchmarks. You will notice that the Naive method provides the best results since all the accuracy metrics are the smallest. We highlighted these results and again made the table more appealing using the `gt` library.
...
Note how the Mean and Naive models predict that the series will continue without a trend. The LS model predicts that the series will continue its trend but, like all other methods, does not consider the seasonal pattern discussed in @sec-Visual. In future chapters, we will look at models that account for trend and seasonality.
...
The graph shows how the LS method does well with the test data and a long forecast period. This can be confirmed by obtaining the accuracy measures against the test set. The code below uses the `accuracy()` function to generate the main table. Once again, the `gt` package is used to enhance the table visually.
...
Interestingly, the Naive method is no longer the best model. This is partly because it chases the series fluctuations (along with the noise) and always makes the same forecast regardless of how many periods we forecast ahead. The LS model captures the trend, which seems to carry consistently throughout the entire series.
...
The Naive method once again performs well. However, we note once again that the Naive method will provide the same forecast for one, two, three or more periods ahead. You can confirm that this model would not be very viable if we are predicting 3 or four periods ahead. Additionally, there is no formal model telling us how data is generated or that explains the how the time series is generated.

## Other Accuracy Measures

AIC (Akaike Information Criterion), AICc (corrected AIC), and BIC (Bayesian Information Criterion) are commonly used measures of model accuracy or goodness of fit in statistical modeling. They are used to compare different models and select the one that best balances model complexity and fit to the data.
...
<center>$AIC = -2 * log-likelihood + 2 * number of parameters$</center>

\n

In this formula, the log-likelihood represents how well the model fits the data, and the number of parameters accounts for the complexity of the model. The lower the AIC value, the better the model is considered in terms of fit and complexity.
...
In this formula, log-likelihood represents the model fit, $n$ is the sample size, and $k$ is the number of parameters. BIC puts a greater emphasis on simplicity compared to AIC. Therefore, when selecting between models, the model with the lower BIC value is considered to be better.

These measures can be easily calculated in R by using the `report()` function. The code below estimates the ETS model (which we will learn in the next module) as well for comparisons, as the naive model is non-parametric and does not provide us with a AIC, AICc, or BIC.
...
calits_train %>%
  ...
  tab_style_body(
    ...
    targets ="row")
```
The model with the lowest AIC (AICc or BIC) is the simple Least Squares model. This indicates that this model outperforms the ETS model for the avocado data set. We should choose this model to forecast avocado prices for Chilango's. 
