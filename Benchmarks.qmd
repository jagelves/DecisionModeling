...
You'll notice that the table records the estimates for all the models discussed is Section $5.1$. However, the naive method has no estimate, as it is simply the previous period's observed value. Below we illustrate the fit of the least squares model by the red line and the Naive model by the orange line.
...
Instead of selecting a fixed training set and test set we can create several. In the code below the first three observations are the training set. A model is then estimated and it forecasts the fourth observation. The error is recorded and the training set is changed again so that now the first four observations are used to estimate the model and forcast the fifth observation. This procedure is repeated as many time as the data allows. 
```{r}
avocado_cv <- calits_train %>%
  stretch_tsibble(.init = 3, .step = 1)
head(avocado_cv,8) %>% gt()
```
Below you can see how the models are estimated and forecasts are generated for $h=1$ (i.e, one period ahead). Then the accuracy is measured and average accross all iterations for each model. The result is shown below:
...
```
The Naive method once again performs well. However, we note that the Naive method will provide the same forecast for one period ahead as for 2, 3 or more periods ahead. Additionally, there is no formal model telling us how data is generated.
...
-   Creating a test set and train set to avoid overfitting
