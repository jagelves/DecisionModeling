...
Instead of selecting a fixed training set and test set we can create several. For example, we could take the first three observations and define them as the training set. We can then estimate a model and forecast the fourth observation. The forecast error is then recorded and the training set is changed so that now the first four observations are used to estimate the model and forecast the fifth observation. This procedure is repeated as many time as the data allows. Below is a table that creates these various training sets.
```{r}
