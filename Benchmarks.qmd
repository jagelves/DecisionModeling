...
```{r}
cali %>%
  as_tsibble(key=c(geography),index=date,regular=T) %>%
...
In this formula, the log-likelihood represents how well the model fits the data, and the number of parameters ($k$) accounts for the complexity of the model. The lower the AIC value, the better the model is considered in terms of fit and complexity.
...
These measures can be easily calculated in R using the `report()` function. The code below estimates the ETS model (which we will learn in the next module) for illustration purposes. Recall that the the naive model is non-parametric and does not provide us with an AIC, AICc, or BIC.
