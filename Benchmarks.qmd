...
coef(fit) %>% gt() %>% 
  ...
  fmt_number(columns =c(statistic,estimate,std.error,p.value),
             decimals = 2)
```
You'll notice that the table records the estimates for all the models discussed is Section $5.1$. However, the naive method has no estimate, as it is simply the previous period's observed value. Below we illustrate the fir of the least squares model by the red line and the Naive model by the orange line.
