...
where $\hat y_{T+h}$ is the predicted value for $h$ periods ahead, and $y_T$ is the value observed at the current time period $T$. We can adjust the Naive prediction by accounting for some natural drift (an increase or decrease). Once again, if we think about weight, we note that as kids grow, we might expect their weight to be close to the previous measurement but slightly higher as we need to account for growth. Mathematically we would say:
...
Now we will use the `model()` function can to run the benchmarks discussed in section 5.1. We have save the models to an object called *fit*.
...
coef(fit)%>% gt() %>% 
  ...
  fmt_number(columns =c(statistic,estimate,std.error,p.value),
             decimals = 2)
```
The table illustrates the estimates for all the models discussed. Note that he Naive method has no estimate, as it is simply the previous periods observed value. Below we illustrate the least squares model by the red line and the Naive model by the orange line.
...
We will assess the fit of the benchmarks by comparing the fitted values against actual values. Generally, a good fit is determined by how far the fitted values are from the actual ones. If we square all of the distances (i.e., errors) and then average them, we calculate the Mean Squared Error (MSE).

<center>
$MSE = \frac{ \sum (\hat{y}_t-y_t)^2}{T-1}$  
