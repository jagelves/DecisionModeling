...
Decision trees map the entire decision process. Below you can see the decision process of trying an addictive brand of chocolate.
...
The decision tree is read from left to right. Notice that there are two main branches stemming from the first decision node (i.e., the first square to the left). You can decide to *try* or *not try*. If you try, you reach another decision node where you can continue purchasing the chocolate or you can stop. In the end, these choices lead to monetary payoffs. For example, if you decide to try and stop you will receive one dollar.

To solve decision trees we use **backward induction**. Start at the end of the tree (right). Choose the branch that leads to the highest payoff for each of the last decision nodes. Repeat procedure while moving backwards to the initial node. Applying this procedure to our simple example, we would start with the decision to *continue* or *stop*. Since stop leads to the higher value ($1$), we should choose stop if we ever reach this decision. Moving back to our initial decision, we now have the choice to *try* or *not try*. In this case we should try, since we would stop in our next choice leading to an outcome of 1 which is greater than not trying (0).
...
-   Learn the concept of expected monetary value

-   Learn how to use decision trees to map the decision process

-   Learn how to use backward induction to determine the optimal decision under uncertainty.
