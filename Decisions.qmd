...
This EMV is higher than the EMV of making the decision without the consultants (1.8 million). This highlights that the information provided by the consulting team is valuable to the CEO. Moreover, the CEO should be willing to pay up to \$1,000,000 (EMV with free information - EMV without information) for the consulting service.

## Readings

The readings for this chapter are mainly from @PMS. Chapter 9 provides a good introduction to decision models with a couple of solved problems using excel. I would recommend using R as a calculator since we will be mainly using R in this book. To construct decision trees in R you can use [mermaid](https://www.rdocumentation.org/packages/DiagrammeR/versions/1.0.10/topics/mermaid) diagrams which can be easily rendered using [Quarto](https://Quarto.org).
